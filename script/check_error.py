...
def static_test():
  ...
  for value in data['report']:
    ...
    # La somma dei casi diagnosticati oggi e del totale dei casi diagnosticati ieri deve essere uguale al totale dei casi diagnosticati oggi.
    if value['tamponi']['totali']['casi_diagnosticati'] != prec_1 + value['tamponi']['odierni']['casi_diagnosticati']:
      error = True
      print('\nFile: %s.json' % value['data'], '=>' , print_error(11))
    prec_1 = value['tamponi']['totali']['casi_diagnosticati']

    # La somma dei casi positivi oggi e del totale dei casi positivi ieri deve essere uguale al totale dei casi positivi oggi.
    if value['tamponi']['totali']['casi_positivi'] != prec_2 + value['tamponi']['odierni']['casi_positivi']:
      error = True
      print('\nFile: %s.json' % value['data'], '=>' , print_error(12))
    prec_2 = value['tamponi']['totali']['casi_positivi']

    # La somma dei decessi per sesso deve essere uguale alla somma totale dei decessi.
    if value['decessi']['totali']['decessi'] != value['decessi']['totali']['sesso']['maschio'] + value['decessi']['totali']['sesso']['femmina']:
      error = True
      print('\nFile: %s.json' % value['data'], '=>' , print_error(13))

    # La somma dei decessi per provincia deve essere uguale alla somma totale dei decessi.
    total_7 = 0
    ...
    # La somma dei decessi odierni e del totale dei decessi di ieri deve essere uguale al totale dei decessi di oggi.
    if value['decessi']['totali']['decessi'] != prec_3 + value['decessi']['odierni']['decessi']:
      error = True
